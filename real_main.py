from src.env import PandaEnv, RealEnv
...
def identify_next_subtask(
    env: RealEnv, gen: LLM, messages: list[dict], verbose: bool, **kwargs
) -> str:
    """Phase 1: Analyzes the environment to determine the next text-based subtask."""
    position, orn = env.get_print_state()
    objs_table = generate_objects_table(env, new_detection=True)
